...
/**
 * Safely formats a date as a locale string, handling invalid dates gracefully
 */
export function formatDateSafe(date: Date | string | null | undefined, fallback: string = "Not set"): string {
...
/**
 * Formats a client's join date, falling back to created_at if joinedAt is null
 */
export function formatClientJoinDate(joinedAt: Date | string | null | undefined, createdAt?: Date | string | null | undefined): string {
  // Try joinedAt first
  if (joinedAt) {
    const dateObj = typeof joinedAt === 'string' ? new Date(joinedAt) : joinedAt;
    if (!isNaN(dateObj.getTime())) {
      return dateObj.toLocaleDateString();
    }
  }
  
  // Fallback to createdAt if joinedAt is invalid
  if (createdAt) {
...
/**
 * Safely formats a date for display, showing year only for older dates
 */
export function formatDateOrYear(date: Date | string | null | undefined, fallback: string = "2023"): string {
  if (!date) return fallback;
